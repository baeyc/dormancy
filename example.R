...
data.and.proba <- dplyr::inner_join(rep.session,probaBB)

library(ggplot2)
ggplot(data=data.and.proba,aes(x=probaBB,y=budburst)) + geom_point()
...
# To get the data of a given species : data.per.species[["carbet"]]
obs.data <- data.per.species[["corave"]]
